  ...
  return (
    <div className="py-2">
      <div className="flex items-center justify-between mb-4 px-1">
        <span className="text-xs font-extrabold tracking-widest uppercase text-[#0055B3]">Filters</span>
        {selectedTags.size > 0 && (
          <button onClick={onClearAll} className="text-xs text-slate-400 hover:text-[#0055B3] font-bold underline">
      ...
      <input
        ...
        className="w-full px-3 py-2 text-xs border border-slate-200 rounded-full bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#0055B3] mb-4 font-semibold"
      />
      ...
      <div className="space-y-1">
        {Object.entries(grouped).map(([group, tags]) => {
          ...
          return (
            <div key={group} className="border-b border-slate-100 pb-1">
              <button
                onClick={() => toggleGroup(group)}
                className="w-full flex items-center justify-between py-2 px-1 text-left hover:bg-blue-50 rounded-lg"
              >
                <span className="text-xs font-extrabold tracking-wide uppercase text-slate-600">{group}</span>
                <span className="flex items-center gap-1.5">
                  {activeCount > 0 && (
                    <span className="bg-[#FFB800] text-[#0055B3] text-xs rounded-full px-2 py-0.5 font-extrabold leading-none">
                      {activeCount}
                    </span>
                  )}
                      ...
                      <button
                        ...
                        className={`text-xs px-2.5 py-1 rounded-full font-bold transition-all ${
                          active
                            ...
                            : 'bg-white text-slate-600 border border-slate-200 hover:border-[#0055B3] hover:text-[#0055B3]'
                        }`}
                      >
                        {tag}
                      </button>
